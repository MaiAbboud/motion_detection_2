...
im2 = cv.imread('/home/mai/Pictures/vlcsnap-2018-12-03-17h39m48s323.jpg')
im2_gray = cv.cvtColor(im2, cv.COLOR_RGB2GRAY)
TP = np.zeros(np.size(im1_gray))
FP = np.zeros(np.size(im1_gray))
FN = np.zeros(np.size(im1_gray))
...
if algorithm == 4:
    ...
    cols = im1.shape[1]
    # result = img2
    result = cv.warpPerspective(im1, M, (im1.shape[1], im1.shape[0]))
    # result[0:img1.shape[0], 0:img1.shape[1]] = img1
    # plt.imshow(result,'gray'),plt.show()

    a = cv.absdiff(im2, im1)
    b = cv.absdiff(im2, result)
    b[im1_gray.shape[0] - margin_x:, :] = 0
    b[:, im1_gray.shape[1] - margin_y:] = 0
    b[:margin_x, :] = 0
    b[:, :margin_y] = 0
